...
export function FaucetDialog({ isOpen, setIsOpen, resyncGame }: FaucetDialogProps) {
  const { user } = usePrivy();
  ...
  const abbreviatedAddress = address ? `${address.slice(0, 4)}...${address.slice(-2)}` : "";
  ...
  const alreadyFunded = parseFloat(formatEther(balance)) >= 0.01;
  ...
  return (
    <AlertDialog open={isOpen}>
      <AlertDialogContent className="bg-yellow-600 w-[95vw] max-w-md sm:max-w-lg rounded-lg px-4 py-6 overflow-y-auto max-h-[90vh]">
        <AlertDialogHeader>
          <AlertDialogTitle className="text-black text-center">You need more ETH to play moves.</AlertDialogTitle>
          <AlertDialogDescription asChild>
            <div className="flex flex-col gap-3 text-sm sm:text-base text-gray-800">
              <div className="flex items-center justify-center gap-2 text-purple-800 break-all">
                <span className="text-gray-800">{`Player: ${abbreviatedAddress}`}</span>
              ...
              <div className="text-purple-800 flex items-center justify-center">
                <span className="text-gray-800">Balance</span>: {formatEther(balance)} ETH
              </div>
              <p className="text-center">
                Fund your player address with testnet ETH directly via your external wallet.
              </p>
            </div>
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
          ...
          <AlertDialogAction asChild>
            <Button
              ...
              asChild
            >
              <a href="https://ethglobal.com/faucet" target="_blank" className="flex items-center">
                <p>Fund via faucet</p>
